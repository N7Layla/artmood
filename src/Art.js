...
class Art extends Component {
  render() {
    ...
    return (
        ...
        <div>{piece.title}, {piece.people ? piece.people.filter(person => person.role === "Artist")[0].displayname : ''}, {piece.dated || piece.century}, {piece.culture}, {piece.technique}</div>
        <div><small>{piece.creditline}</small></div>
        <div><a href={piece.url}>Read More</a></div>
        </div>
